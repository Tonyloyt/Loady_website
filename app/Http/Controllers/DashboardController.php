...
class DashboardController extends Controller
{
   public function dashboard(Request $request){
	...
	$clientsorders = DB::select("select * from `clientsorders` where clientid='".$id."'");
   ...
   $data = [
				...
				'doorbell' => $res->doorbell,
				'rushservice_washfold' => $res->rushservice_washfold,
				'rushservice_drycleaning' => $res->rushservice_drycleaning, 
   ...
   return view('accounts.clientdashboard')
	...
	if ($validator->fails()) {
		...
		if($request->has('rushservice_washfold')){
			$rs=$data2['rushservice_washfold'];
		}
		$rd="";
		 if($request->has('rushservice_drycleaning')){
			$rd=$data2['rushservice_drycleaning'];
		 }
	...
	if ($validator->fails()) {
		...
		try{
			...
			if( $data['scheduleddate']!=""){
			      ...
			      //GET CLIENT ID FROM MAIL
					$user2 = DB::select("select * from `users` where email='".Session::get('loginmail')."'"); 
			...
			//return response()->json(['success'=>$data['scheduleddate']]);
			 
			
					$pickup = new clientsorders;
					$pickup->scheduleddate = $data['scheduleddate'];
					$pickup->drycleaning = $data['drycleaning'];
					$pickup->washfold = $data['washfold'];
					$pickup->hangdry = $data['hangdry'];
					$pickup->status='Waiting Pickup';
					$pickup->clientid=$id;
				
				
                     $stm="insert into clientsorders( scheduleddate, drycleaning,washfold,hangdry,clientid,status)values('".$data['scheduleddate']."','".$data['drycleaning']."','".$data['washfold']."','".$data['hangdry']."','".$id."','Waiting Pickup');";
			...
			//return response()->json(['success'=>"ss"]);
...
public function removepayment(Request $request){
	$id=$request->get('id');
	$rm = DB::delete("delete from paymentmethod where id='".$id."'");
