...
var year = $(this).val(); 
var validName = /^[0-9]+$/; 
var validrange = /^[2020-2025]+$/;
if (year.length == 0) {
$('.year-msg').addClass('invalid-msg').text("Year is required");
$(this).addClass('invalid-input').removeClass('valid-input'); 
} 
else if (!validrange.test(year)) {
$('.year-msg').addClass('invalid-msg').text('valid range [2020-2025]'); 
$(this).addClass('invalid-input').removeClass('valid-input'); 
} 
...
var validName = /^[0-9]+$/; 
if (apt .length == 0) {
$('.apt-msg').addClass('invalid-msg').text("apt  is required");
$(this).addClass('invalid-input').removeClass('valid-input'); 
} 
else if (!validName.test(apt )) {
$('.apt-msg').addClass('invalid-msg').text('only digits are allowed'); 
$(this).addClass('invalid-input').removeClass('valid-input'); 
} 
else { 
$('.apt-msg').empty();
...
//Validation for paymentmethod_apt 
$('#paymentmethod_apt').on('input', function () { 
var paymentmethod_apt  = $(this).val(); 
var validName = /^[0-9]+$/; 
if (paymentmethod_apt .length == 0) {
$('.paymentmethod_apt-msg').addClass('invalid-msg').text("apt  is required");
$(this).addClass('invalid-input').removeClass('valid-input'); 
} 
else if (!validName.test(paymentmethod_apt)) {
$('.paymentmethod_apt-msg').addClass('invalid-msg').text('only digits are allowed'); 
$(this).addClass('invalid-input').removeClass('valid-input'); 
} 
else { 
$('.paymentmethod_apt-msg').empty();
$(this).addClass('valid-input').removeClass('invalid-input'); 
} 
});
...
else if (paymentmethod_zipcode.length > 4) {
$('.paymentmethod_zipcode-msg').addClass('invalid-msg').text('4 digits maximum'); 
$(this).addClass('invalid-input').removeClass('valid-input'); 
} 
		...
		<div style="display:block; padding-top:10px;">
			<div class="loadly_rinsedrop1">
				...
				<div style="padding-top: 20px; display: inline-block;" class="loadly_contents_fonts" class="loadly_contents_fonts"><span> When you enable Rinse drop, you won't need to present for your pickups &  deliveries. There is no more convinient way to Rinse </span></div>
				 <div style="padding-top: 30px; display: none;" class="loadly_contents_fonts"> <a href="#">Activate Rinse Drop</a></div>
		    </div>
               ...
               jQuery.ajax({
                  ...
                  success: function(result){
					 ...
					 if(result.success == "Successful Updated"){
						$('#success_payment').html("just popup");
